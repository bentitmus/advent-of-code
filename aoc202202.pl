...
% Transform A, B, C and X, Y, Z into 1, 2, 3 for a single item
transform_score((OpponentChar, StrategyChar), (Opponent, Strategy)) :-
   Opponent is OpponentChar - 64,
   Strategy is StrategyChar - 87.

% True if the Opponent and Strategy would produce the given score for the match
match_score(Opponent, Opponent, 3) :- !.
match_score(Opponent, Strategy, 6) :- Strategy is (Opponent rem 3) + 1, !.
match_score(Opponent, Strategy, 0) :- Strategy is ((Opponent + 1) rem 3) + 1, !.
